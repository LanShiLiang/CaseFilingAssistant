...
- 产品仅面向中国大陆法律及司法规则。
- MVP 聚焦根据用户上传的证据、调解书、判决书等材料，生成可人工审阅的强制执行申请材料包。
...
- 桌面 Web 是 MVP 的基准体验；移动 Web 必须用同一业务流程完成核心闭环。微信小程序是后续实现，届时保持适用门禁、来源、确认、阻断、人工复核和未提交边界一致，但不得直接复用 DOM UI 或在小程序端复制服务端法律规则。
...
- 服务端状态与未提交表单分离。可达步骤、校验结果、生成资格和权威金额由服务端响应派生；浏览器本地状态只管理未提交输入和展示状态，不得建立第二套业务状态机。上传、轮询或其他推进 revision 的后台动作必须按 dirty 字段重放未提交输入，禁止用刷新后的服务端快照整表覆盖。
