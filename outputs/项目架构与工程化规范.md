...
- 已有产品、前端、后端方案和可审核设计稿。
- `apps/web`、`services/api`、`packages/contracts`、`infra` 等仅为目录占位。
- 尚无可运行的 Next.js、FastAPI、worker、数据库迁移、Compose、依赖锁或测试代码。
- 因此当前**不能**执行一键启动，也不得对外宣称 `docker compose up --build` 已可用。

本规范中的目录、命令和门禁除明确标注“当前已有”外，均为下一阶段实现目标。
...
- 文书只能读取已确认、指定 revision 的不可变快照，不能直接读取 OCR 候选或浏览器表单草稿。
...
├─ compose.yaml                    # 实现并验收后加入
├─ package.json                    # pnpm/Turbo 根任务；实现后加入
├─ pnpm-workspace.yaml
├─ pnpm-lock.yaml
├─ turbo.json
├─ .dockerignore
...
这些选择影响实现和发布，但不改变“草稿、人工复核、无对外提交”的产品边界。
