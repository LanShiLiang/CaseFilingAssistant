...
| 步骤 | 后端完成条件 | 失败处理 |
|---|---|---|
| 1 当事人与依据 | 有一个已通过上传格式门禁并成功保存的执行依据；文书类型、案号和作出法院已确认；申请执行人与被执行人身份证正反面均已上传；申请执行人身份字段和唯一被执行人姓名已确认；无范围阻断 | “已保存”不要求 OCR/规则成功；解析失败时开放固定字段手工录入，缺失字段或稳定 blocker 仍阻断下一步 |
| 2 申请内容 | 单一金钱义务、金额、请求事项、申请法院和送达地址已确认；义务来自执行依据候选，或由用户查看执行依据后手工录入并确认 | OCR/规则失败不构成硬门禁；不自动推定金额、利率、法院或法律结论 |
...
身份证 OCR 失败时，只要对应当事人的身份证正反面原件已经存在，用户仍可手工录入并确认身份字段。申请执行人与被执行人身份证双面均为必传材料，被执行人姓名必须确认。
...
P0 每个事项只允许一个 active `legal_basis`，以及申请执行人、被执行人各一份 active 身份证人像面和国徽面。替换不是覆盖原 Blob：服务端创建新文档版本、推进 `data_revision`、使旧来源引用失效，再由清理任务删除不可达 Blob。`performance_evidence` 仅为旧数据兼容保留，当前 UI 不再提供上传入口。
...
- 当前 UI 不采集履行情况材料；旧事项中的 `performance_evidence` 仅按兼容数据读取，不作为新流程入口。
...
- P0 拒绝 ZIP。
- `document_kind` 与格式白名单匹配：执行依据允许 PDF/DOCX/JPG/PNG，身份证正反面只允许 JPG/PNG；旧 `performance_evidence` 类型继续兼容读取，但当前 UI 不允许新建。
