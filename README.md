...
- 流式上传 PDF、DOCX、JPG、PNG，并校验大小、签名和 DOCX 压缩包边界；执行依据负责提取案号、法院、日期、金额，以及原告/被告的姓名和身份证号候选，并分别映射为申请执行人/被执行人字段；扫描 PDF/图片在本地 OCR 可用时进入 OCR，失败时允许人工补充。
- 三步闭环：资料填写（当事人、执行依据与请求信息）→ 检查与预览 → 导出；执行依据及申请执行人、被执行人身份证双面均为必传材料。步骤 1 主按钮会先在 Modal 中按模块列出缺失项，再以“收集—保存—检查”进度锁定表单；预览生成期间同样显示任务进度，避免 revision 冲突和无反馈等待。
- 字段保存材料 ID、解析 revision、来源位置、置信度和人工确认状态；版本化 `DossierV2` 统一约束 JSON 数据。步骤保存会统一确认本次提交的非空字段，材料替换或数据变化会提升 revision，并使旧来源、旧校验、旧预览和旧导出失效。
...
- 桌面 Web 与移动 Web 使用同一业务流程；步骤门禁由服务端统一返回，API 通过带大小限制、取消传播和超时的 Next.js 流式同源代理访问。写请求使用稳定幂等键；身份证号和证件记载住址默认遮罩，联系电话、送达地址、收款账户和财产线索按表单要求明文展示；不启用 Redux DevTools。
